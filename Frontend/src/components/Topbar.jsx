...
import {
  Moon, Sun, Code2, ChevronDown, Check,
  Zap, FolderOpen, Clock, GitFork, Loader2
} from 'lucide-react'
import { Button } from '@/components/ui/button'

// Topbar — brand, language selector, theme toggle, and all feature action buttons.
// Props:
//   brand, language, onLanguageChange, onReview, snippets (existing)
//   showLivePanel, onToggleLive
...
export default function Topbar({
  brand = 'CodeNest',
  language, onLanguageChange, onReview, snippets,
  showLivePanel, onToggleLive,
  showProjectSidebar, onToggleProjects,
  showVersionPanel, onToggleVersions,
  isVisualizing, onVisualize,
  ...
  const handleReview = async () => {
    setIsReviewing(true)
    try { await onReview() } finally { setIsReviewing(false) }
  }
  ...
  return (
    <div className="sticky top-4 z-50 mx-4 my-2 flex items-center justify-between px-4 py-2.5 rounded-2xl bg-white/70 dark:bg-black/60 backdrop-blur-2xl border border-white/20 dark:border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.12)] dark:shadow-[0_8px_32px_rgba(0,0,0,0.4)] transition-all duration-500 hover:shadow-[0_12px_40px_rgba(0,0,0,0.15)] dark:hover:shadow-[0_12px_40px_rgba(0,0,0,0.5)] gap-2">
      ...
      {/* Right side: Language + Theme + Review */}
          ...
          <div className={`absolute top-full right-0 mt-2 w-44 p-1 rounded-xl bg-white/80 dark:bg-black/80 backdrop-blur-xl border border-white/20 dark:border-white/10 shadow-2xl origin-top-right transition-all duration-200 z-50 ${isLangOpen ? 'opacity-100 scale-100 translate-y-0' : 'opacity-0 scale-95 -translate-y-2 pointer-events-none'}`}>
            {languages.map(lang => (
              <button
                ...
                className={`flex items-center justify-between w-full px-3 py-2 my-0.5 text-xs rounded-lg transition-colors ${language === lang.id ? 'bg-violet-500/10 text-violet-600 dark:text-violet-400 font-medium' : 'hover:bg-secondary/50 text-foreground/80'}`}
              >
                {lang.label}
