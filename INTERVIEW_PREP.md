...
│   └── src/
│       ├── main.jsx               # React DOM concurrent root mounting entry
│       ├── index.css              # Global Tailwind + custom scrollbars
│       │
│       ├── app/
│       │   ├── App.jsx            # Core bootstrapper with smooth scroll initializers
│       │   ├── router.jsx         # Express-like client-side route configurations
│       │   └── providers.jsx      # Aggregated context and error boundaries
│       │
│       ├── features/
│       │   ├── auth/
│       │   │   ├── components/
│       │   │   │   ├── AuthDialog.jsx     # Login/register modal gateway dialog
│       │   │   │   └── ProtectedRoute.jsx # Route guarding session validator
│       │   │   ├── hooks/
│       │   │   │   └── useAuth.js         # Session context consumer hook
│       │   │   └── api.js                 # Encapsulated auth API gateway
│       │   │
│       │   ├── editor/
│       │   │   ├── EditorPanel.jsx        # Multi-language syntax editor
│       │   │   ├── hooks/                 # Future editor-specific hook bindings
│       │   │   └── utils/                 # Future editor helper abstractions
│       │   │
│       │   ├── ai/
...
│       │   │
│       │   ├── projects/
│       │   │   ├── ProjectSidebar.jsx     # File explorer tree & ZIP downloader
│       │   │   └── useProjectStore.js     # MongoDB workspace state synchronizer
│       │   │
│       │   └── versioning/
│       │       ├── VersionTimeline.jsx    # Client timeline revision controller
│       │       └── DiffViewer.jsx         # Custom side-by-side LCS diff visualizer
│       │
│       ├── components/
│       │   ├── ui/
│       │   │   └── button.jsx             # Primitive buttons (shadcn-style)
│       │   │
│       │   └── common/
│       │       ├── Topbar.jsx             # Branding navbar, toggles, language picker
│       │       ├── DebugPanel.jsx         # Stderr tracebacks + AI auto-fix buttons
│       │       ├── ErrorBoundary.jsx      # Crash recovery layout boundary
│       │       ├── theme-provider.jsx     # Context toggling dark/light skins
│       │       ├── LandingPage.jsx        # Cinematic presentation hero dashboard
│       │       ├── Desktop.jsx            # macOS-style window desktop layout shell
│       │       └── desktop/
│       │           ├── MenuBar.jsx        # Top macOS header strip
│       │           ├── Dock.jsx           # Magnified dock app launcher
│       │           ├── Finder.jsx         # User guide file finder window
│       │           └── DesktopIcon.jsx    # Icon shortcuts triggering launches
│       │
│       ├── pages/
│       │   ├── Landing.jsx            # Aggregates hero page with login dialogs
│       │   ├── DesktopView.jsx        # Cinematic boot-loader and desktop shell
│       │   └── DashboardView.jsx      # Heavy IDE workspace panel grid layout
│       │
│       ├── hooks/
│       │   └── useWorkspace.js        # Core domain coordinator linking all panels
│       ├── services/
│       │   └── api.js                 # Axios instance with core client endpoints
│       ├── context/
│       │   └── AuthContext.jsx        # Centralized login session state manager
│       ├── lib/
...
### 6.1 App Bootstrap, URL Routing & guards (`src/app/` + `ProtectedRoute.jsx`)

CodeNest decouples its application initialization and routing tables under a dedicated `src/app/` structure:

- **App Bootstrapper (`src/app/App.jsx`):** Initialized inside Vite's concurrent mounting layer (`src/main.jsx`). It instantiates global animation layouts, including the **Lenis smooth-scrolling engine** to provide buttery transitions across presentation states, and mounts providers and route nodes.
- **Providers Layer (`src/app/providers.jsx`):** Aggregates all global contexts in a single nested layer, wrapping components sequentially inside custom `ThemeProvider`, `AuthProvider`, and error-isolating `ErrorBoundary` wrapper shells.
- **Routing Manifest (`src/app/router.jsx`):** Defines the central client-side SPA route mappings:
  - `/` ➔ Guest landing presentation view (`Landing.jsx`)
  - `/desktop` ➔ macOS-style desktop launcher view (`DesktopView.jsx`)
  - `/dashboard` ➔ High-performance developer IDE workspace view (`DashboardView.jsx`)

**Access Gate (`src/features/auth/components/ProtectedRoute.jsx`):** Gated navigation guard that secures private browser views (`/desktop`, `/dashboard`). It reads active JWT status from the centralized context layer. If bootstrap checks are actively hydrating credentials from persistent cache pools, it displays a premium dark spinner. If no active session token is present upon validation, it triggers a clean client-side redirect back to the home landing page.

### 6.2 Authentication State & Sessions (`AuthContext.jsx` + `src/features/auth/`)

The global auth state machine is managed by **`src/context/AuthContext.jsx`**, utilizing endpoints isolated inside the **`src/features/auth/api.js`** gateway and exposed via the custom **`useAuth` hook** (relocated to `src/features/auth/hooks/useAuth.js`):

1. **Bootstrap Sequence:** On mount, the provider calls `fetchHealth()` to verify API connectivity, then queries `getCurrentUserApi()` to hydrate token credentials from local storage cache. If the token is expired or unauthorized, it gracefully clears caching (`clearStoredSession()`) and resets workspace contexts.
2. **Query Param Bootstrap:** An active observer extracts single-use `resetToken` query parameters from the URL list, automatically popping up the `AuthDialog` recovery modal to let candidates update credentials instantly.

### 6.3 Decoupled State & Workspace Orchestrator (`src/hooks/useWorkspace.js`)

Historically, all editor buffers, compile terminal logs, and AI panel data lived as a monolithic block inside `App.jsx`. CodeNest decoupled this into a dedicated **`useWorkspace.js` domain hook** to split visual page presentation (`DashboardView.jsx`) from logic:

- **State Scope:** Manages editor `code`, `language`, console `output` collections, compiler `stderrLines`, AI `review` feeds, visual `mermaidDiagram` strings, and panel toggles (`showProjectSidebar`, `showVersionPanel`, `showDebugPanel`).
- **State Hooks Integration:** Coordinates and consumes domain-specific state machines:
  - `useAuth()` to query candidate profile configurations.
  - `useProjectStore()` to sync and auto-save code buffers.
  - `useVersionStore()` to track local in-browser git snapshots.
- **Debounced Processes:**
  - **Auto-save** (2000ms): keystrokes clear `saveDebounceRef` and trigger optimistic updates via `useProjectStore` to sync code back to MongoDB Atlas in real-time.
  - **Live check** (2000ms): keystrokes trigger debounced AI code metrics and live suggestions via the `liveCheck` api.

### 6.4 Handlers Encapsulation

Core logic routines are fully encapsulated within `useWorkspace.js` and exposed directly to layout views:

| Handler | Chronological Flow & Logic |
|---------|---------------------------|
| `runCode` | Saves current code snapshot ➔ Calls execution API ➔ Formats stdout/stderr ➔ Emits compiler regex warnings. |
| `reviewCode` | Snapshot ➔ Triggers Groq SDK analysis ➔ Updates tabbed markdown and visual flowchart graphs. |
| `applyPrompt` | Sends natural-language instruction ➔ Strips AI markdown fences ➔ Programmatically injects new code. |
| `handleAutoFix` | Submits stderr traceback to AI ➔ Returns clean diagnostic fix ➔ Applies and creates recovery snapshot. |
| `handleVisualize` | Submits source code ➔ Converts logic to Mermaid flowchart nodes ➔ Updates panning Canvas boards. |
| `handleRestore` | Backs up current editor draft ➔ Hydrates past local snapshot from timeline memory. |
...
### 6.7 `useProjectStore.js` (`src/features/projects/useProjectStore.js`)
...
### 6.8 `useVersionStore.js` (`src/features/versioning/useVersionStore.js`)
...
#### `Topbar.jsx` (`src/components/common/Topbar.jsx`)
...
#### `EditorPanel.jsx` (`src/features/editor/EditorPanel.jsx`)

- `react-simple-code-editor` — textarea + highlight overlay
- `prism.highlight` per language grammar
- Run button → `onRun`

#### `ReviewPanel.jsx` (`src/features/ai/ReviewPanel.jsx`)
...
#### `TerminalPanel.jsx` (`src/features/terminal/TerminalPanel.jsx`)

- Renders `output[]` lines
- Error styling if line matches Error/Traceback/Exception
- "Debug with AI" when `hasError`

#### `PromptPanel.jsx` (`src/features/ai/PromptPanel.jsx`)
...
#### `ProjectSidebar.jsx` (`src/features/projects/ProjectSidebar.jsx`)
...
#### `VersionTimeline.jsx` (`src/features/versioning/VersionTimeline.jsx`)
...
LandingPage ➔ AuthDialog ➔ POST /api/auth/register
  ...
  ➔ navigate('/desktop') ➔ Click vscode icon
  ➔ navigate('/dashboard') ➔ useProjectStore loads GET /api/projects
```
...
**Q: How does the application organize routes and route protection?**  
**A:** It uses React Router (`react-router-dom`) with three core pages: `Landing.jsx`, `DesktopView.jsx`, and `DashboardView.jsx`. Private routes are secured using a `ProtectedRoute.jsx` component that listens to a reactive `AuthContext` provider. If a user is unauthenticated or the session hydration fails, it intercepts access and issues a server-grade history-clean redirect back to the home page.

**Q: How is complex IDE state managed without cluttering presentation views?**  
**A:** We decoupled visual presentation from state by introducing a custom **`useWorkspace.js` domain hook**. This central orchestrator handles all real-time editor state changes, debounced auto-saves, sandboxed compilation executions, and Groq SDK AI review requests. The page view components like `DashboardView.jsx` simply consume this hook, keeping them visual, clean, and highly reusable.
...
- [ ] useWorkspace.js state orchestration + debouncing
